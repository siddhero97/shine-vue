  ...
  <ion-page>
    ...
    <ion-content>
      <div id="container" style="text-align: left">
        <ion-button @click="createCategory()" style="float: right">+ Create Project (Category)</ion-button>
        <ion-title size="medium">Project (Category)</ion-title>
        <ion-grid  style="margin-top: 2em;">
          <ion-row style="font-weight: bold" class='ion-align-items-center'>
            <ion-col size="4">ID - Description</ion-col>
            <ion-col size="">Order</ion-col>
            <ion-col size="3">&nbsp;</ion-col>
          </ion-row>
          
          <ion-row v-for="category in categories" v-bind:key="category.id" class='ion-align-items-center'>
            <ion-col size="4">{{ category.id }} - {{ category.description }}</ion-col>
            <ion-col size="">{{ category.order }}</ion-col>
            <ion-col size="3">
              <ion-button @click="editCategory(category.id)">Edit</ion-button>
              <ion-button @click="deleteCategory(category.id, category.description)">Delete</ion-button>
            </ion-col>
          </ion-row>
        </ion-grid>
      </div>
